function dan_plus(varargin)
% -------------------------------------------------------------------------
%prepare the data
% -------------------------------------------------------------------------

load('C:\Study\Research\Computer Vision\Introduction to Computer Vision\Project\Data\num_ext_frames\10\fea_video_train_plus.mat');
fea_video_train_plus=reshape(fea_video_train_plus,[1,1,size(fea_video_train_plus,1),size(fea_video_train_plus,2)]);
load('C:\Study\Research\Computer Vision\Introduction to Computer Vision\Project\Data\num_ext_frames\10\labels_video_train.mat') ;
...
num_train=size(fea_video_train_plus,4);

load('C:\Study\Research\Computer Vision\Introduction to Computer Vision\Project\Data\num_ext_frames\10\fea_video_val_plus.mat');
fea_video_val_plus=reshape(fea_video_val_plus,[1,1,size(fea_video_val_plus,1),size(fea_video_val_plus,2)]);
load('C:\Study\Research\Computer Vision\Introduction to Computer Vision\Project\Data\num_ext_frames\10\labels_video_val.mat') ;
...
num_val=size(fea_video_val_plus,4);

imdb.meta.sets={'train','val'};
imdb.images.data=single(cat(4, fea_video_train_plus, fea_video_val_plus));
...
trainOpts.gpus = [];
trainOpts.learningRate = 0.001 ;
trainOpts.expDir = 'exp\dan_plus_10' ;
trainOpts.errorFunction = 'none' ;
trainOpts = vl_argparse(trainOpts, varargin);
...
save('exp\dan_plus_10\dan_plus_10.mat', '-struct', 'net') ;
...
net = load('exp\dan_plus_10\dan_plus_10.mat') ;

% Load the test data
load('C:\Study\Research\Computer Vision\Introduction to Computer Vision\Project\Data\num_ext_frames\10\fea_video_test_plus.mat');
fea_video_test_plus=reshape(fea_video_test_plus,[1,1,size(fea_video_test_plus,1),size(fea_video_test_plus,2)]);
load('C:\Study\Research\Computer Vision\Introduction to Computer Vision\Project\Data\num_ext_frames\10\labels_video_test.mat') ;

test_data = single(fea_video_test_plus);
...
test_pred=test_pred';
csvwrite('exp\dan_plus_10\result_dan_plus_10.csv',test_pred);
diff = test_pred - labels_video_test;
diff = abs(diff);
err_mean=mean(diff(:));
accuracy=1-err_mean;
fprintf('Test accuracy of dan_plus is %.3f%%.', accuracy);
